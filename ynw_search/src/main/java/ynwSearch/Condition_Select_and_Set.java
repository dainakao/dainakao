...
public class Condition_Select_and_Set extends JFrame {
	...
	//GUIを設定・表示
	public void guiSelectSet(String pass_name, String[] untreated_files, String[] files, String[][] dataName){
		...
		JRadioButton radio[] = new JRadioButton[untreated_files.length];
		int counter = 0;
		for(int i=0; i<untreated_files.length; i++) {
			for(int j=0; j<files.length; j++) {
				if(files[j].equals(untreated_files[i])) {
					break;
				}
				if(j==files.length-1) {

					radio[counter] = new JRadioButton(untreated_files[i]);
					radio[counter].setFont(new Font("ＭＳ ゴシック", Font.PLAIN, 2*fontsize));
					radio[counter].setPreferredSize(new Dimension(4*radio[counter].getUIClassID().length()*fontsize, 4*fontsize));// ボタン追加
					p1.add(Box.createRigidArea(new Dimension(0,distance)));
					p1.add(radio[counter]);
					bgroup1.add(radio[counter]);
					counter++;
				}
			}
		}
				...
				for(int i=0; i<radio.length; i++) {
					...
					if (radio[i].isSelected()){
						for(int j=0; j<untreated_files.length; j++) {
							if(radio[i].getText().equals(untreated_files[j])) {
								dispose();
								f.Start_Condition_Select_and_Set(j, pass_name, untreated_files, files, dataName);
							}
						}
					}
				}
	...
	//修正画面を表示
	public void dataSelectSet(int untreated_files_number, String pass_name, String[] untreated_files, String[] files, String[][] dataName){
		int fontsize = 12;//フォントサイズ

		//データ配列を宣言、初期化（すべてfalse）
		boolean[][] data = new boolean[dataName.length+1][dataName[0].length];
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0 ; i < radio4_U.length; i++){
					...
					if (i==radio4_U.length-1) {
						if(safetySelectSet(untreated_files_number, pass_name, data, untreated_files, files, dataName) == true){
							System.out.println();
							dispose();
							//ファイルをリストアップ
	...
	public boolean safetySelectSet(int number, String pass_name, boolean[][] data, String[] untreated_files, String[] files, String[][] dataName) {
		boolean status = true;
		...
		if(status==true) {
			dataEntrySelectSet(number, pass_name, untreated_files[number], data,untreated_files, files, dataName);
	...
	public void dataEntrySelectSet(int number, String pass_name, String file_name, boolean[][] data, String[] untreated_files, String[] files, String[][] dataName) {
		//ファイルを作成
