...
public class Functions {
	...
	//GUI分類を起動
	public void Start_Setting(int number, String pass_name, String[] untreated_files, String[] files, String[][] dataName) {
		Condition_Setting cs = new Condition_Setting();   // ウィンドウ作成
		for(int i=0; i<files.length; i++) {
			if(files[i].equals(untreated_files[number])) {
				number++;
				i=-1;
			}
		}
