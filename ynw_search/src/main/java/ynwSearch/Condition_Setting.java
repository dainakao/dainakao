...
public class Condition_Setting extends JFrame {
	...
	//GUIを設定・表示
	public void guiInput(int number,String pass_name, String[] untreated_files, String[] files, String[][] dataName){
		...
		for(int i=0; i<C; i++) {
			...
			if(i<C/2) {
				if(i==0) {
					...
					Yes_No.setFont(new Font(Yes_No.getFont().getName(), Font.BOLD, fontsize));
		...
		//演目表示ラベル追加
		label = new JLabel(untreated_files[number]);
		label.setFont(new Font(label.getFont().getName(), Font.BOLD, 2*fontsize));
		...
		btn3.setFont(new Font(btn3.getFont().getName(), Font.BOLD, 2*fontsize));
