...
public class TF_IDF {
	...
	//検索→TF_IDFを用いて順位付け
	//2字語句を新インデックスから検索
	public void search_TF_IDF_aaa(String file_name, String pass_name, String search_word) {
		if(search_word.equals("")) {
			System.out.println("検索語が入力されていません");
		}else {
			double[][] data = null;//0;TF_IDF値 1;ファイルの番号 2;出現位置
			...
			double[][] answers = new double[search_words.length][files.length];//部分検索の合致リスト
			//初期化
				...
				for(int i=0; i<indexs.size(); i++) {
					if(indexs.get(i).get(0).equals(search_words[h])) {
						for(int j=1; j<indexs.get(i).size(); j=j+2) {
							answers[h][Integer.parseInt(indexs.get(i).get(j))] = Double.parseDouble(tf_idf.get(i).get(Integer.parseInt(indexs.get(i).get(j))));;
						}
					}
				}
			}
