...
public class Functions {
	...
	public static String body_scr2(String str) {
		...
		try {
			...
			//クラス名を指定して書き出す
			Elements elm = doc.getElementsByClass("mw-parser-output");
	...
	public static String[][] list_scr(String str) {
		...
		for (Element link : links) {
			if(status == 1) {
				if(link.text().contentEquals("前のページ")||link.text().contentEquals("次のページ"))break;
				title.add(link.text());
				url.add(link.attr("href"));
			}if(status == 2) {
				if(link.text().contentEquals("次のページ")) {
					status = 1;
				}else {
					status = 3;
				}
				...
				if(link.text().contentEquals("前のページ")) status = 2;
			}
		}
